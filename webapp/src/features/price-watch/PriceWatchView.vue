...
function recognize() {
  ...
  notice.value = `已识别京东商品，SKU：${jdSku.value}`
}
  ...
  <main class="price-watch-page">
    ...
    <template v-else>
      ...
      <section class="panel add-panel">
        ...
        <div class="recognition-box" :class="{ recognized: validUrl }">
          <template v-if="validUrl">
            ...
            <div class="recognized-copy">
              ...
              <span>SKU：{{ jdSku }} · 商品名称、图片和实时价格后续接入。</span>
            </div>
        ...
        <p class="fine-print">ⓘ 当前版本会保存京东 SKU、购买价和监控周期；实时价格检测和提醒后续接入。</p>
      </section>
      ...
      <div class="content-grid">
        ...
        <aside class="panel guide-panel">
          ...
          <ol>
            ...
            <li><b>识别 SKU</b><span>系统从商品链接中提取京东 SKU</span></li>
            <li><b>设置购买价和监控天数</b><span>选择 7 天 / 15 天 / 30 天</span></li>
            <li><b>开始监控</b><span>保存至数据库，价格检测和提醒后续接入</span></li>
          </ol>
          <div class="tip">
            <b>💡 温馨提示</b>
            <span>1. 当前仅对 admin 用户开放使用。</span>
            <span>2. 当前先保存商品和监控信息，暂不自动查询价格。</span>
