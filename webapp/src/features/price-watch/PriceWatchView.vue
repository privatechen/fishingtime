...
async function removeItem(item: WatchItem) {
  ...
  try {
    const response = await fetch(`/api/price-watch/${item.watchId}`, {
      method: 'DELETE',
      credentials: 'same-origin',
    })
    const json = await response.json()
            ...
            <article v-for="item in items" :key="item.watchId" class="watch-item" :class="{ unavailable: !item.available }">
              ...
              <div class="item-main">
                <div class="item-title-row">
                  ...
                  <span v-if="!item.available" class="unavailable-chip">当前商品不可查</span>
                </div>
              ...
              <button class="remove-btn" :disabled="removingWatchId === item.watchId" @click="removeItem(item)">{{ removingWatchId === item.watchId ? '移除中...' : '移除' }}</button>
            </article>
          </div>
        </section>
