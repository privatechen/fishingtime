/**
 * 2048 游戏引擎 — 纯 TypeScript 逻辑，不依赖 Vue
 *
  ...
  isGameOver(): boolean {
    for (let r = 0; r < this.size; r++) {
      for (let c = 0; c < this.size; c++) {
        ...
        if (c + 1 < this.size && this.grid[r][c]!.value === this.grid[r][c + 1]!.value) return false
        if (r + 1 < this.size && this.grid[r][c]!.value === this.grid[r + 1][c]!.value) return false
      }
    }
    return true
  }
