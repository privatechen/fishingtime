...
public class MyGameRecordService {
    ...
    public List<GameRecordDTO> getMyRecords(Long userId){List<GameRecordDTO> l=new ArrayList<>();l.add(build2048(userId));l.add(buildColorFocus(userId));l.add(buildDirectionTrap(userId));l.add(buildColorHunter(userId));l.add(buildDontFill(userId));l.add(buildStackTower(userId));l.add(buildOneStroke(userId));l.add(buildNumberPuzzle(userId));l.add(buildMemory(userId));l.add(buildFishBreakout(userId));l.add(buildExtremeFishing(userId));return l;}
    ...
    private GameRecordDTO buildMemory(Long id){GameRecordDTO d=new GameRecordDTO();d.setGameType("memory");MemoryScore s=memoryScoreMapper.selectByUserId(id);if(s!=null){d.setMemoryBestPassedCount(s.getBestPassedCount());d.setMemoryBestStreak(s.getBestStreak());d.setMemoryMaxStage(s.getMaxStage());}return d;}
