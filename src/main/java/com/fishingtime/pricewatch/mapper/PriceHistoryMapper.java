...
public interface PriceHistoryMapper {
    ...
    @Select("SELECT DATE(ph.checked_at) AS priceDate, ph.price AS price " +
            ...
            "  FROM price_history " +
            "  WHERE platform = 'JD' AND product_id = #{productId} " +
            ...
            "WHERE ph.platform = 'JD' AND ph.product_id = #{productId} " +
            "ORDER BY ph.checked_at ASC")
    List<PriceHistoryPointRow> findDailyLastPrices(@Param("productId") Long productId,
                                                   @Param("days") int days);
